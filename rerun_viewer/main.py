...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Also visualize Feather Sense sensor data (USB serial or BLE, see "
        "--feather-transport). Default: auto (use it if the device is detected); "
        "--feather requires it (error if absent); --no-feather disables the probe.",
    )
    parser.add_argument(
        "--feather-transport",
        choices=["serial", "ble"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.feather is not False:  # None (auto) or True (required)
        ...
        if feather is None and args.feather is True:
            ...
            return 1
        print(
            f"Feather Sense ({args.feather_transport}) detected on {feather.port}; streaming sensor data."
